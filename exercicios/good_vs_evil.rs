...
// Input:
// The function will be given two parameters. Each parameter will be a string of multiple integers separated by a 
//single space. Each string will contain the count of each race on the side of good and evil.
...
fn good_vs_evil(good: &str, evil: &str) -> String {
    ...
    let good:i32 = good.split_whitespace()
    .zip(good_worth.iter())
    .map(|(word, worth)| word.parse::<i32>().unwrap() * worth)
    .sum();

    let evil:i32 = evil.split_whitespace()
    .zip(evil_worth.iter())
    .map(|(word, worth)| word.parse::<i32>().unwrap() * worth)
    .sum();
...
fn good_vs_evil_comunidade(good: &str, evil: &str) -> String {
